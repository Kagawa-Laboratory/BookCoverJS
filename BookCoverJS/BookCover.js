export const BookCover = (function () {
...
//    self.__root;
  ...
  BookCover.prototype = {
    ...
    xorsRand: function() {
      const self = this; 
      const t = self.__xors.x ^ (self.__xors.x << 11);
    ...
    hsb1tohsl1: function(h, s, b) {
        ...
        const luminance = max - diff;
        const saturation = luminance <= 0.5 ? diff / luminance : diff / (luminance - 1);
    ...
    hsl255: function(h, s, l) {
        return this.hsl1(h / 255, s / 255, l / 255);
    },
    ...
    rgb2hsb360: function(rgb) {
        const b = rgb % 0x100;
        const g = rgb / 0x100 % 0x100;
        const r = rgb / 0x10000 % 0x100;
        let max, min, sub, angle;
    ...
    rotateH360: function(color, a) {
        const hsb = this.rgb2hsb360(color);
        hsb[0] += a;
    ...
    addS100L: function(color, a) {
        const hsl = this.rgb2hsl360(color);
        hsl[1] += a;
    ...
    //    printf("<g transform=\"translate(%f,%f)\">\n", x, y);
        const group = this.__currentGroup.group();
        group.translate(x, y);
    ...
    arc: function(x, y, w, h, start, stop) {
        ...
        const x0 = x + w  / 2 + w / 2 * Math.cos(start);
        const y0 = y + h / 2 + h / 2 * Math.sin(start);
        const x1 = x + w / 2 + w / 2 * Math.cos(stop);
        const y1 = y + h / 2 + h / 2 * Math.sin(stop);
    ...
    //    printf("  fill=\"%s\" fill-opacity=\"%f\"", __fill, __fillOpacity);
    //    printf("  stroke-linecap=\"round\" />\n");
        const pathStr = this.myVsprintf("M%f,%f A%f,%f  0 %d,%d %f,%f",
    ...
    //    printf("  fill=\"none\"");
    //    printf("  stroke-linecap=\"round\" />\n");
        const pathStr = this.myVsprintf("M%f,%f C%f,%f %f,%f %f,%f"
    ...
    text: function() {
        ...
        const str = this.myVsprintf.apply(null, as);
    ...
    rulers: function() {
        const tmpW = this.__strokeWeight;
        const tmpC = this.__stroke;
    ...
    pageFrame: function()  {
        const tmpW = this.__strokeWeight;
        const tmpO = this.__strokeOpacity;
        const tmpC = this.__stroke;
        const tmpF = this.__fill;
        this.strokeWeight(0.2);
    ...
    trimMark: function() {
        ...
        if (this.__landscape) {
            ...
            this.line(200 * sx, 10 * sy, 180 * sx, 10 * sy);
        }
 ...
 */  
    ...
    genericSmilieSymbol: function(callback) {
        const ret = this.__currentGroup.symbol().viewbox(0, 0, 5000, 5000);
    ...
    //    printf("<path d=\"M 2500,0 L 2762,13 3014,49 3257,109 3488,191 3708,294 3914,417 4105,559 4281,719 4441,895 4583,1086 4706,1292 4809,1512 4891,1743 4951,1986 4987,2238 5000,2500 4987,2762 4951,3014 4891,3257 4809,3488 4706,3708 4583,3914 4441,4105 4281,4281 4105,4441 3914,4583 3708,4706 3488,4809 3257,4891 3014,4951 2762,4987 2500,5000 2238,4987 1986,4951 1743,4891 1512,4809 1292,4706 1086,4583 895,4441 719,4281 559,4105 417,3914 294,3708 191,3488 109,3257 49,3014 13,2762 0,2500 13,2238 49,1986 109,1743 191,1512 294,1292 417,1086 559,895 719,719 895,559 1086,417 1292,294 1512,191 1743,109 1986,49 2238,13 2500,0 Z\"/>\n");
        const g1 = symbol.group().attr({ 'stroke': "none", 'fill': "black" });
    ...
    //    printf("</g>\n");
        const g2 = symbol.group().attr({ 'stroke': "none", 'fill': "black" });
    //    printf("<g style=\"stroke:none;fill:black\">\n");
    ...
    //    printf("</g>\n");
        const g3 = symbol.group().attr({ 'stroke': "black", 'stroke-width': 200, 'fill': "none" });
    ...
    smilieSymbol1: function() {
        return this.genericSmilieSymbol(function (g) {
 ...
 */
    ...
    finish: function() {
        this.penUp();
        this.resetMatrix();
        const draw = this.__currentGroup; 
//        draw.size(this.__width, this.__height);
    ...
    clipWithCard: function(card) {
        const rect = this.__currentGroup
    ...
    numCards: function() {
        if (this.__cards) return this.__cards.length;
        return 1;   // カードなし
    },
